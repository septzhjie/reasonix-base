...
def main():
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        # 进度条监控[11](@ref)
        with tqdm(total=len(video_files), desc="生成封面") as pbar:
            for future in as_completed(future_to_video):
                ...
                try:
                    ...
                    if success:
                        success_count += 1
                        pbar.set_postfix_str(f"成功：{Path(video).name}")
                    else:
                        pbar.set_postfix_str(f"失败：{Path(video).name} - {error}")
                except Exception as e:
                    pbar.set_postfix_str(f"异常：{Path(video).name} - {str(e)}")
                finally:
                    pbar.update(1)
